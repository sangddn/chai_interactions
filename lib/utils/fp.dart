// Extension methods for functional programming in Dart.
// Copyright 2024 Sang Doan.

part of 'utils.dart';

T identityFn<T>(T t) => t;
...
extension MapToList<K, V> on Map<K, V> {
  ...
  Iterable<R> expandIter<R>(Iterable<R> Function(K, V) fn) {
    return entries.expand((entry) => fn(entry.key, entry.value));
  }
